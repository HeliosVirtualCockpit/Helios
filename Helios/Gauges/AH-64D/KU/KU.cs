...
namespace GadrocsWorkshop.Helios.Gauges.AH64D.KU
{
    ...
    public class KU : CompositeVisualWithBackgroundImage
    {
            ...
            : base(interfaceDevice, new Size(600, 465))
        {
            ...
            _frameBezelPanel = AddPanel("KU Frame", new Point(Left, Top), NativeSize, "{Helios}/Gauges/AH-64D/KU/KU_Frame.png", _interfaceDevice);
        ...
        private void AddButton(string name, Rect rect, string label)
        {
            ...
            string image = (label == "N" || label == "S" || label == "E" || label == "W") ? $"{{Helios}}/Gauges/AH-64D/KU/tactile-dark-{shape}-emphasised" : $"{{Helios}}/Images/Buttons/tactile-dark-{shape}";
            switch (label)
            {
                case "+/-":
                    label = "±";
            ...
            if (label != "")
            {
                ...
                if (boostSize) button.TextFormat.FontSize = 32; else button.TextFormat.FontSize = 16;
                if (boostSize) {
