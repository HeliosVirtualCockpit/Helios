...
function driver.processLowImportance(mainPanelDevice)		
	...
	if li then
        for i=0, 5 do
        helios.send(2400 + i, string.format("%s;%s;%s;%s", i+1,helios.ensureString(li[oduVarNames[(3*i) + 1]]):gsub(":","!"),helios.ensureString(li[oduVarNames[(3*i) + 2]]):gsub(":","!"),helios.ensureString(li[oduVarNames[(3*i) + 3]]):gsub(":","!")))
        end
    end

    helios.send(2010, string.format("%.0f",mainPanelDevice:get_argument_value(368) * 100000+mainPanelDevice:get_argument_value(369) * 10000+mainPanelDevice:get_argument_value(370) * 1000))    -- Fuel Total
    helios.send(2011, string.format("%.0f",mainPanelDevice:get_argument_value(373) * 10000+mainPanelDevice:get_argument_value(374) * 1000))                                                     -- Fuel Left Tank
    helios.send(2012, string.format("%.0f",mainPanelDevice:get_argument_value(377) * 10000+mainPanelDevice:get_argument_value(378) * 1000))                                                     -- Fuel Right Tank    
