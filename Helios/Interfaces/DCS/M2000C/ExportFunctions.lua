...
function driver.processLowImportance(mainPanelDevice)
    ...
    if li then
        ...
        helios.send(2297, string.format("%-6s", helios.ensureString(li.PCN_UL_POINTS)))
        ...
        helios.send(2289, string.format("%0.1f", li["PCN_UL_N"] and 1 or 0))
        helios.send(2290, string.format("%0.1f", li["PCN_UL_S"] and 1 or 0))
        helios.send(2291, string.format("%0.1f", li["PCN_UR_E"] and 1 or 0))
        helios.send(2292, string.format("%0.1f", li["PCN_UR_W"] and 1 or 0))
        helios.send(2293, string.format("%0.1f", li["PCN_UL_P"] and 1 or 0))
        helios.send(2294, string.format("%0.1f", li["PCN_UL_M"] and 1 or 0))
        helios.send(2295, string.format("%0.1f", li["PCN_UR_P"] and 1 or 0))
        helios.send(2296, string.format("%0.1f", li["PCN_UR_M"] and 1 or 0))
    end    
