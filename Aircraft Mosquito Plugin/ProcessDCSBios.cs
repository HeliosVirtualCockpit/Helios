namespace GadrocsWorkshop.Helios.Interfaces.DCS.Mosquito
{
    ...
    public class ProcessDCSBios
	{
		public ProcessDCSBios()
		{
			string pattern = @"define(?<function>[a-zA-Z\d]*)\((?<arg>[a-zA-Z\d_\x22\x20\-\{\}\.]+)?(,[\s]*(?<arg>[a-zA-Z\d_\x22\x20\-\{\}\.\/\)\(]+))+\)[\s-]*\n";
			string input = @"BIOS.protocol.beginModule(""Mosquito"", 0x7000)
...
defineToggleSwitch(""HARNESS_LVR"", 5, 3043, 323, ""Lower Fuselage Part"", ""Harness Lever"")
defineToggleSwitch(""HYDR_SEL"", 2, 3069, 333, ""Lower Fuselage Part"", ""Hydraulic Select"")
...
defineToggleSwitch(""LOOP_ANT_LOCK"", 25, 3107, 241, ""Quarter"", ""Loop Antenna Lock"")
defineRotary(""LOOP_ANT_SCALE"", 25, 3094, 240, ""Quarter"", ""Loop Antenna Scale"")
defineTumb(""IFF_CHAN"", 4, 3104, 188, 0.2, {0, 1}, nil, false, ""Quarter"", ""IFF Channel Switch"")
...
BIOS.protocol.endModule()";
			...
			{
				string addFn = m.Groups["function"].Value;
                var argArray = m.Groups["arg"].Captures.OfType<Capture>().Select(c => c.Value).ToArray();
                switch (addFn)
                {
                    case "PushButton":
                        Console.WriteLine("AddFunction(new {0}(this, \"{1}\", \"{3}\", \"{2}\", {4}, {5}, \"%1d\"));   // {6}", addFn, argArray[1], argArray[3], argArray[2], argArray[4], argArray[5], argArray[0]);
                        break;
                    case "Potentiometer":
                        Console.WriteLine("AddFunction(new Axis(this, \"{0}\", \"{1}\", \"{2}\", 0.1d, {3}d, {4}d, {5}, {6}, false, \"%.1f\"));   // {7}", argArray[1], argArray[2], argArray[3], argArray[4].Substring(1), argArray[5].Substring(0, 1), argArray[6], argArray[7], argArray[0]);
                        break;
                    case "IndicatorLight":
                        Console.WriteLine("AddFunction(new FlagValue(this, \"{0}\", {1}, {2}, \"True when indicator is lit\", \"%1d\"));   // {3}", argArray[1], argArray[2], argArray[3], argArray[0]);
                        break;
                    case "ToggleSwitch":
                        Console.WriteLine("AddFunction(Switch.CreateToggleSwitch(this, \"{1}\", \"{3}\", \"{2}\", \"1\", \"Position 1\", \"0\", \"Position 2\", {4}, {5}, \"%1d\"));   // {6}", addFn, argArray[1], argArray[3], argArray[2], argArray[4], argArray[5], argArray[0]);
                        break;
                    case "MultipositionSwitch":
                        Console.WriteLine("AddFunction(new Switch(this, \"{1}\", \"{2}\", SwitchPositions.Create({4}, 0.0, {5}, \"{3}\", \"Position \", \"%.2f\"), {6}, {7}, \"%.2f\"));   // {6}", addFn, argArray[1], argArray[3], argArray[2], argArray[4], argArray[5], argArray[6], argArray[7], argArray[0]);
                        break;
                    case "3Pos2CommandSwitchWW2":
                        Console.WriteLine("AddFunction(new Switch(this, \"{1}\", \"{4}\", new SwitchPosition[] {{new SwitchPosition(\"1.0\", \"Position 1\", \"{2}\"), new SwitchPosition(\"0.0\", \"Position 2\", \"{2}\"), new SwitchPosition(\"-1.0\", \"Position 3\", \"{3}\")}}, {5}, {6}, \"%.1f\"));   // {7}", addFn, argArray[1], argArray[2], argArray[3], argArray[4], argArray[5], argArray[6], argArray[0]);
                        break;
                    case "Rotary":
                        Console.WriteLine("AddFunction(new RotaryEncoder(this, \"{1}\", \"{3}\", \"{2}\", 0.01d, {4}, {5}));   // {6}", addFn, argArray[1], argArray[3], argArray[2], argArray[4], argArray[5], argArray[0]);
                        break;
                    case "3PosTumb":
                        Console.WriteLine("AddFunction(Switch.CreateThreeWaySwitch(this, \"{1}\", \"{2}\", \"{3}\", \"1\", \"Position 1\", \"0\", \"Position 2\", \"-1\", \"Position 3\", {4}, {5}, \"%1d\"));   // {6}", addFn, argArray[1], argArray[3], argArray[2], argArray[4], argArray[5], argArray[0]);
                        break;
                    case "Tumb":
                        double switchValue = double.Parse(argArray[4]);
                        double switchMinValue = double.Parse(argArray[5].Replace("{", ""));
                        double switchMaxValue = double.Parse(argArray[6].Replace("}", ""));
                        double switchPositions = ((switchMaxValue-switchMinValue) / switchValue)+1;
                        Console.WriteLine("AddFunction(new Switch(this, \"{1}\", \"{2}\", SwitchPositions.Create({7}, {8}, {9}, \"{3}\", \"Position \", \"%.2f\"), {4}, {5}, \"%.2f\"));   // {6}", addFn, argArray[1], argArray[3], argArray[2], argArray[9], argArray[10], argArray[0], switchPositions, switchMinValue, switchValue);
                        break;
                    case "Float":
                        Console.WriteLine("AddFunction(new NetworkValue(this, \"{1}\", {4}, {5}, \"\", \"Numeric Value between {2} and {3}\", BindingValueUnits.Numeric));   // {6}", addFn, argArray[1], argArray[2].Replace("{",""), argArray[3].Replace("}",""), argArray[4], argArray[5], argArray[0]);
                        break;
                    case "IntegerFromGetter":
                        //Console.WriteLine("AddFunction(new {0}(this, \"{1}\", \"{3}\", \"{2}\", {4}, {5}, \"%1d\"));   // {6}", addFn, argArray[1], argArray[3], argArray[2], argArray[4], argArray[5], argArray[0]);

                        break;
                    default:
                        // if(!functionList.Contains(addFn)) functionList.Add(addFn);	

                        break;
				}
                // Remove these comments after Moss
				// Console.WriteLine("AddFunction(new {0}(this, {1}, {2}, {3}));   // {6}", m.Groups["function"].Value, argArray[1], argArray[3], argArray[2], argArray[4], argArray[5], argArray[0]);
			}
