namespace GadrocsWorkshop.Helios.Interfaces.DCS.DH98Mosquito
{
    ...
    public static class ProcessDCSBios
	{
		public static Dictionary<string, string> GetFunctions()
		{
            Dictionary<string,string> functions = new Dictionary<string,string>();

			string pattern = @"define(?<function>[a-zA-Z_\d]*)\((\x22)?(?<arg>[a-zA-Z\d_\&\x20\-\{\}\.]+)?(\x22)?([\s]*,[\s]*(\x22)?(?<arg>[a-zA-Z\d_\&\x20\-\{\}\.\/\)\(]+)(\x22)?)+\)[\s-]*\n";
			string input = @"--Main Pit
...
";
			...
			foreach (Match m in Regex.Matches(input, pattern, options))
			{
                ...
                if (addFn != "IntegerFromGetter")
                {
                    string argNum = string.Empty;
                    string description = string.Empty;
                    if (argArray.Count() >= 5)
                    {
                        if(addFn != "Float")
                        {
                            //Console.WriteLine("\"{2}\", \"{3}\", \"{4}\"", argArray[1], argArray[2], argArray[3], argArray[argArray.Count() - 2], argArray[argArray.Count() - 1]);                            if (!functions.ContainsKey(argArray[3]))
                            argNum = argArray[3];
                            description = $"\"{argArray[argArray.Count() - 2]}\", \"{argArray[argArray.Count() - 1]}\"";
                        } else
                        {
                            //Console.WriteLine("\"{0}\", \"{3}\", \"{4}\"", argArray[1], argArray[2], argArray[3], argArray[argArray.Count() - 2], argArray[argArray.Count() - 1]);
                            argNum = argArray[1];
                            description = $"\"{argArray[argArray.Count() - 2]}\", \"{argArray[argArray.Count() - 1]}\"";
                        }
                    } else
                    {
                        //Console.WriteLine("\"{0}\", \"{1}\", \"{2}\"", argArray[1], argArray[argArray.Count() - 2], argArray[argArray.Count() - 1]);
                        argNum = argArray[1];
                        description = $"\"{argArray[argArray.Count() - 2]}\", \"{argArray[argArray.Count() - 1]}\"";
                    }

                    if (!functions.ContainsKey(argNum))
                    {
                        functions.Add(argNum, description);
                    } else
                    {
                        Console.WriteLine("* * * original \"{0}\", {1}", argNum, functions[argNum]);
                        Console.WriteLine("* * * duplicate \"{0}\", {1}", argNum, description);
                    }
    ...
    //                    break;
    //                case "Rotary":
    //                    Console.WriteLine("//AddFunction(new Rotary(this, \"{1}\", \"{3}\", \"{2}\", {4}, {5}, \"%1d\"));   // {6}", addFn, argArray[1], argArray[3], argArray[2], argArray[4], argArray[5], argArray[0]);
    //                    break;
    //                case "3PosTumb":
    //                    Console.WriteLine("//AddFunction(new {0}(this, \"{1}\", \"{3}\", \"{2}\", {4}, {5}, \"%1d\"));   // {6}", addFn, argArray[1], argArray[3], argArray[2], argArray[4], argArray[5], argArray[0]);
    ...
    //                    break;
    //                case "Float":
    //                    Console.WriteLine("//AddFunction(new NetworkValue(this, \"{1}\", \"{3}\", \"{2}\", {4}, {5}, \"%1d\"));   // {6}", addFn, argArray[1], argArray[3], argArray[2], argArray[4], argArray[5], argArray[0]);
    ...
    //                    break;
                ...
                // Console.WriteLine("{1}, {2}, {3}));   // {6}", m.Groups["function"].Value, argArray[1], argArray[3], argArray[2], argArray[4], argArray[5], argArray[0]);
