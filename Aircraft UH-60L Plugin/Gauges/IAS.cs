...
namespace GadrocsWorkshop.Helios.Gauges.UH60L.Instruments
{
    ...
    [HeliosControl("Helios.UH60L.IAS", "Indicated Airspeed", "UH-60L Blackhawk", typeof(BackgroundImageRenderer), HeliosControlFlags.NotShownInUI)]
    public class IAS : CompositeVisualWithBackgroundImage
    {
        private string _interfaceDeviceName = "IAS";

        private static readonly Rect SCREEN_RECT = new Rect(0, 0, 1, 1);
        private Rect _scaledScreenRect = SCREEN_RECT;
        private IASInstrument _iasInstrument;

        public IAS()
            : base("Indicated Airspeed", new Size(420,420))
        {
            SupportedInterfaces = new[] { typeof(Interfaces.DCS.UH60L.UH60LInterface) };
            _interfaceDeviceName = "IAS";
        ...
        {
            _iasInstrument = new IASInstrument(name, size)
            {
                Top = pos.Y,
                Left = pos.X,
            };
            Children.Add(_iasInstrument);
            ...
            //}
            foreach (IBindingAction action in _iasInstrument.Actions)
            {
                if(action.Name != "hidden")
                {
                AddAction(action, _iasInstrument.Name);
                //Create the automatic input bindings for the sub component
                AddDefaultInputBinding(
                   childName: _iasInstrument.Name,
                   deviceActionName: _iasInstrument.Name +"." + action.ActionVerb + "." + action.Name,
