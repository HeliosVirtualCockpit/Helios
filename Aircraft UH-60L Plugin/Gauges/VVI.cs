...
namespace GadrocsWorkshop.Helios.Gauges.UH60L.Instruments
{
    ...
    [HeliosControl("Helios.UH60L.VVI", "Vertical Velocity Indicator", "UH-60L Blackhawk", typeof(BackgroundImageRenderer), HeliosControlFlags.NotShownInUI)]
    public class VVI : CompositeVisualWithBackgroundImage
    {
        private string _interfaceDeviceName = "VVI";

        private static readonly Rect SCREEN_RECT = new Rect(0, 0, 1, 1);
        private Rect _scaledScreenRect = SCREEN_RECT;
        private VVIInstrument _VVIInstrument;

        public VVI()
            : base("Vertical Velocity", new Size(300,300))
        {
            SupportedInterfaces = new[] { typeof(Interfaces.DCS.UH60L.UH60LInterface) };
            _interfaceDeviceName = "VVI";
            AddPart("Instrument", new Point(0d, 0d), new Size(300d, 300d), _interfaceDeviceName, "Instrument");
        }
        private void AddPart(string name, Point pos, Size size, string interfaceDevice, string interfaceElement)
        {
            _VVIInstrument = new VVIInstrument(name, size)
            {
                Top = pos.Y,
                Left = pos.X,
            };
            Children.Add(_VVIInstrument);
            ...
            //}
            foreach (IBindingAction action in _VVIInstrument.Actions)
            {
                if(action.Name != "hidden")
                {
                AddAction(action, _VVIInstrument.Name);
                //Create the automatic input bindings for the sub component
                AddDefaultInputBinding(
                   childName: _VVIInstrument.Name,
                   deviceActionName: _VVIInstrument.Name +"." + action.ActionVerb + "." + action.Name,
